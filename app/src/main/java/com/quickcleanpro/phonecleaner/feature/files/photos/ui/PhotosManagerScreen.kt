package com.quickcleanpro.phonecleaner.feature.files.photos.ui
...
import com.quickcleanpro.phonecleaner.feature.files.photos.PhotosManagerUiState
...
import com.quickcleanpro.phonecleaner.feature.files.shared.ui.FileManagerGalleryBrowserView
import com.quickcleanpro.phonecleaner.common.format.FileSizeFormatter

@Composable
internal fun PhotosManagerScreen(
    state: PhotosManagerUiState,
    ...
    val currentItems = state.currentDisplayItems
    ...
    FileManagerScaffold(
        ...
        actions = {
            val actionText = when (state.phase) {
                ...
                FileOperationPhase.ConfirmDelete ->
                    stringResource(if (state.allSelected) R.string.file_unselect_all else R.string.file_select_all)
                else -> null
            }
            FileManagerTopAction(actionText = actionText) { onAction(FileManagerAction.ToggleVisibleItems) }
        },
        bottomBar = {
            if (permissionGranted && state.phase == FileOperationPhase.Browsing && !isDetailMode) {
                CleanXBottomActionBar(
                    ...
                    text = if (state.selectedIds.isNotEmpty()) {
                        stringResource(R.string.file_delete_size, FileSizeFormatter.format(state.selectedSizeBytes))
        ...
        Column(modifier = modifier.background(FileManagerPageBrush).padding(horizontal = 16.dp)) {
            FileOperationPhaseContent(
                ...
                FileManagerGalleryBrowserView(
                    ...
                    onSelectAll = { onAction(FileManagerAction.ToggleVisibleItems) },
                    onOpenDetail = { item ->
