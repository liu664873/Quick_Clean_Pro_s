...
import com.quickcleanpro.phonecleaner.feature.files.videos.ui.VideosManagerScreen
...
internal fun VideosManagerRoute(
    ...
    VideosManagerRouteContent(navigator = navigator, viewModel = viewModel, featureFlow = featureFlow)
}

@Composable
private fun VideosManagerRouteContent(
    navigator: AppNavigator,
    viewModel: VideosManagerViewModel,
    featureFlow: FeatureFlowRuntime,
) {
        ...
        onAction = { action ->
            when (action) {
                ...
                is FileManagerAction.OpenDetail -> {
                    ...
                    router.openRoute(AppDestination.VideosDetail.withDetailInitialIndex(action.index))
                }
                else -> Unit
            }
        },
