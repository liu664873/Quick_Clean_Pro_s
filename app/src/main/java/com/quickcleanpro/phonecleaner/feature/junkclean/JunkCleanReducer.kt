...
object JunkCleanReducer {
    fun reduce(state: JunkCleanUiState, action: JunkCleanAction): JunkCleanUiState =
        when (action) {
            ...
            is JunkCleanAction.StopDialogShown ->
                state.copy(showStopDialog = true, stopDialogPhase = action.phase)

            JunkCleanAction.StopDialogDismissed ->
                state.copy(showStopDialog = false, stopDialogPhase = null)
        }
    ...
    private fun buildPreviewGroups(result: com.quickcleanpro.phonecleaner.feature.junkclean.model.ScanResult) =
