...
import com.quickcleanpro.phonecleaner.feature.files.photos.ui.PhotosManagerScreen
...
internal fun PhotosManagerRoute(
    ...
    PhotosManagerRouteContent(navigator = navigator, viewModel = viewModel, featureFlow = featureFlow)
}

@Composable
private fun PhotosManagerRouteContent(
    navigator: AppNavigator,
    viewModel: PhotosManagerViewModel,
    featureFlow: FeatureFlowRuntime,
) {
        ...
        onAction = { action ->
            when (action) {
                ...
                is FileManagerAction.OpenDetail -> {
                    ...
                    router.openRoute(AppDestination.PhotosDetail.withDetailInitialIndex(action.index))
                }
                else -> Unit
            }
        },
